...
| | |
|---|---|
| 🔗 **Live demo** | [Results showcase on Hugging Face Spaces](https://huggingface.co/spaces/MounishAllam/audio-temporal-reasoning-showcase) — real charts, real metrics, real sample outputs, no signup needed |
| 📦 **Trained weights** | [qwen2.5-omni-audio-temporal-qlora on Hugging Face](https://huggingface.co/MounishAllam/qwen2.5-omni-audio-temporal-qlora) — the real 3,000-sample production adapter, downloadable, with usage instructions |
| 📊 **Training data** | Real AudioCaps clips (YouTube audio + human captions). Three independent scaling runs — **300 / 1,000 / 3,000** training clips — each evaluated against its own disjoint **100 / 200 / 300**-clip test set, with a train/test leakage guard enforced in code |
| 💰 **Cost** | **$0.** No paid compute anywhere in this pipeline — 4-bit QLoRA makes the 7B model trainable on a free Kaggle T4 GPU (30 free GPU-hours/week; see [KAGGLE_GUIDE.md](KAGGLE_GUIDE.md)); the public demo above runs on Hugging Face's free CPU tier |
| ✅ **Results** | Real evaluation only, three times over, at increasing data scale — hallucination rate 86.7% → 77.7%, ROUGE-L 12.0% → 31.9% (base → fine-tuned, best run). No simulated numbers anywhere in this repo — see [Results](#results) |
...
— browse the real results without installing anything. (It shows precomputed
results rather than live inference, since Qwen2.5-Omni-7B needs a GPU the
free Space tier doesn't have — run `app_gradio.py` locally, or follow
[KAGGLE_GUIDE.md](KAGGLE_GUIDE.md), for live inference on real audio.)
...
`outputs/benchmark_report_*.json` — re-run it after any new eval and the
chart updates from the real numbers, never hand-edited.*
...
Full per-run metrics, training hyperparameters, and weight-availability status
for all three runs live in **[MODELS.md](MODELS.md)** — the model registry.
Only the 3,000-sample run (`production`) has surviving weights and is marked
deployable; the smaller runs' weights were lost to an overwrite bug (see
MODELS.md's ["Weight loss history"](MODELS.md#weight-loss-history) for why).
...
Real predictions from `checkpoints/final_adapter_3000samples`
(`outputs/predictions_sft_v3.json`), not cherry-picked for this one section —
first clean/mixed examples pulled in file order, shown as-is:
...
| "A person snoring with another man speaking" | "A man speaks and a pig oinks" | Gets the speech right, hallucinates "pig oinks" for the snoring — a real, current failure mode |

That third row is representative of why hallucination rate is still ~78%
even after fine-tuning — see [Failure analysis](#failure-analysis) below for
...
│   ├── make_results_chart.py       Regenerates assets/scaling_curve.png from real eval JSON
...
## Model quality — what would actually move the numbers next

Ranked by expected payoff vs. effort, based on what the three scaling runs
already revealed (see [Failure analysis](#failure-analysis) and the scaling
curve above):

1. **Fix the checkpoint-selection bug (highest value, least effort).**
   `scripts/run_finetune.py` currently saves the model state after the
   *last* epoch, not the one with the best validation loss. All three runs
   overfit between epoch 1 and their final epoch — e.g. the 3,000-sample
   run's val loss went 1.59 → 1.68 from epoch 1 to 2 while train loss kept
   dropping. The trainer already computes and saves
   `checkpoints/checkpoint-best`; it's just never promoted to the final
   adapter. Every number in this README is therefore from a slightly
   overfit checkpoint the pipeline had already discarded a better one for.
2. **Reduce overfitting directly** — raise `lora_dropout` above the current
   0.05, or lower the learning rate, so the epoch-2 validation loss stops
...
3. **More epochs, not more raw clips.** Data scale gave diminishing
   ...
   training samples, but only 0.3 points going from 1,000→3,000. The model
   plateaued on data volume before it plateaued on training signal; more
   epochs at the existing 1,000–3,000-sample scale (combined with #2 to
   control overfitting) is a more promising lever than gathering more clips.
4. **Address the precision/recall trade-off.** Fine-tuning made the model
   ...
   whether the training data's "answer in one plain sentence" style is
   teaching the model to omit real sounds along with invented ones.
5. **Expand the hallucination-detection vocabulary**
   (`evaluation/metrics.py`, ~130 words currently). This doesn't change the
   model, but it changes how honestly it can be measured — captions using
   sound words outside that vocabulary are currently invisible to the
   hallucination metric either way (missed inventions and missed correct
   mentions both undercounted equally).
6. **DPO stays experimental, deliberately.** It's text-only, so it can
   shape caption *style* but can't teach real audio grounding — not worth
